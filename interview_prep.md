...
A: 这是一个智能代码助手 Agent。用户可以用自然语言问关于代码库（TinyDB 开源项目）的问题，Agent 通过 ReAct 循环自主决定调用哪个工具（搜索代码、解释函数、生成测试），最终给出答案。整个过程有对话记录，支持多轮上下文，用户可以点赞/点踩反馈。
...
A: 我构建了 20 条中文测试 query，每条标注了期望命中的源文件，用 Hit Rate / MRR / NDCG 三个指标评估。真实跑出来的结果：

| 配置 | Hit Rate | MRR | NDCG |
|------|----------|-----|------|
| dense only | 0.80 | 0.70 | 0.69 |
| sparse only | 0.80 | 0.70 | 0.69 |
| hybrid (RRF) | 0.80 | 0.70 | 0.69 |
| hybrid + HyDE | 0.85 | 0.72 | 0.72 |
| hybrid + HyDE + rerank | 0.75 | 0.59 | 0.57 |

（注：具体数字以你本地最后一次 run_eval 结果为准，面试前重跑一遍确认）

结论有两个：
1. **HyDE 有效**：加了 HyDE 后 Hit Rate 从 0.80 提到 0.85，但延迟从 0.2ms 涨到 8 秒（每次要调 LLM），所以只适合离线/高价值查询。
2. **reranker 在我们场景降了**：cross-encoder/ms-marco-MiniLM-L-6-v2 是在网页搜索（MS MARCO）上训练的，对代码检索不敏感，把正确结果排后面去了。这是一个有价值的负面结论——说明 reranker 不是加了就一定好，要看领域匹配。
